...
def train_model(model, train_loader, valid_loader, num_epochs, opt, ckpt_path, test_loader=None, test_window_loader=None, save_model=False, data_config=None, fold=None):
    ...
    for i in range(1, num_epochs + 1):
        ...
        if model.model_name=='rkt':
            auc, acc, _, _, _ = evaluate(model, valid_loader, model.model_name, rel)
        else:
            auc, acc, _, _, _ = evaluate(model, valid_loader, model.model_name)
        ### atkt 有diff， 以下代码导致的
        ...
        if auc > max_auc+1e-3:
            ...
            if not save_model:
                if test_loader != None:
                    ...
                    testauc, testacc, _, _, _ = evaluate(model, test_loader, model.model_name, save_test_path)
                if test_window_loader != None:
                    save_test_path = os.path.join(ckpt_path, model.emb_type+"_test_window_predictions.txt")
                    window_testauc, window_testacc, _, _, _ = evaluate(model, test_window_loader, model.model_name, save_test_path)
